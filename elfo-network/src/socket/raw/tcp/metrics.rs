...
pub(crate) fn collect_tcp_metrics<T: AsRawFd>(socket: &T) -> Result<TcpMetrics, std::io::Error> {
    ...
    #[cfg(not(target_os = "linux"))]
    {
        Err(std::io::Error::new(
            ...
            "TCP_INFO not supported on this platform",
        ))
    }
}

macro_rules! tcp_counter {
    ($current:ident.$metric:ident,$prev:ident) => {
        let label =
            concat!("elfo_network_tcp_", stringify!($metric));
        metrics::counter!(label,
            $current.$metric.overflowing_sub($prev.$metric).0 as u64
        );
    };
}
...
pub(crate) fn report_tcp_metrics(current: &TcpMetrics, previous: &TcpMetrics) {
    ...
    tcp_counter!(current.unacked, previous);
    tcp_counter!(current.sacked, previous);
    tcp_counter!(current.pmtu, previous);
    tcp_counter!(current.rcv_space, previous);
    tcp_counter!(current.total_retrans, previous);
    tcp_counter!(current.lost, previous);
}
...
mod tests {
    ...
    fn test_tcp_metrics_default() {
        ...
        assert_eq!(metrics.total_retrans, 0);
    ...
    #[test]
    fn test_collect_metrics_invalid_fd() {
    ...
    #[test]
    fn test_metrics_reporting_does_not_panic() {
    ...
    #[test]
    fn test_tcp_info_collection_with_real_socket() {
        use std::{
