    ...
    pub(crate) fn collect_transport_specific_metrics(&mut self) {
        #[cfg(target_os = "linux")]
        {
            match metrics::collect_tcp_metrics(self.inner.as_ref()) {
                Ok(current_metrics) => {
                    metrics::report_tcp_metrics(&current_metrics, &self.previous_metrics);
                    self.previous_metrics = current_metrics;
                }
                Err(err) => {
                    tracing::warn!(
                        message = "failed to collect TCP_INFO metrics",
                        error = %err,
                    );
                }
            }
...
fn prepare_stream(stream: TcpStream) -> Result<Socket> {
    ...
    let write = OwnedWriteHalf {
        inner: write,
        #[cfg(target_os = "linux")]
        previous_metrics: TcpMetrics::default(),
    };
