...
pub struct MessageVTable {
    ...
    #[cfg(feature = "network")]
    pub read_msgpack:
        unsafe fn(&[u8], NonNull<MessageRepr>) -> Result<(), rmp_serde::decode::Error>,
    #[cfg(feature = "network")]
    #[allow(clippy::type_complexity)]
    pub write_msgpack: unsafe fn(
        NonNull<MessageRepr>,
        LimitedWrite<&mut Vec<u8>>,
...
pub mod vtablefns {
    ...
    // fn erase(message: &internal::AnyMessage) ->
    // crate_::dumping::ErasedMessage {
    // smallbox!(Clone::clone(cast_ref(message)))
    //}

    // fn deserialize_any(deserializer: &mut dyn
    // internal::erased_serde::Deserializer<'_>) -> Result<internal::AnyMessage,
    // #internal::erased_serde::Error> {
    // internal::erased_serde::deserialize::<name>(deserializer).
    // map(crate_::Message::upcast)
    //}
